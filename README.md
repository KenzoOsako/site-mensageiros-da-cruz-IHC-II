...
Em 23/09/2026, um teste temporário no **Supabase hospedado** passou 29 verificações com coordenação e dois participantes fictícios: login por API, criação de ação/tarefa/material, RLS, privacidade das confirmações e ofertas, tarefa exclusiva, recebimento parcial restrito à coordenação, persistência e bloqueio de membro inativo. As três contas e a ação de teste foram removidas. Uma segunda conta fictícia testou recuperação por token, troca de senha e novo login; também foi removida. Pela **interface publicada**, a primeira coordenação consumiu o convite, definiu a própria senha, entrou em `/painel`, saiu e entrou novamente; a sessão continuou válida após recarregar. Uma ação claramente fictícia foi criada pela interface, com participação, tarefa concluída, material, oferta e recebimento parcial. Todos os estados persistiram após recarregar. A ação e seus registros dependentes foram removidos; a agenda voltou a zero ações. Um token de convite deliberadamente inválido foi rejeitado com orientação para pedir novo link. Ainda faltam convite de participantes pela interface e casos de cancelamento/atualização desatualizada no serviço hospedado. O teste de token inválido não simula a passagem real do prazo de expiração.
...
- [x] Verificar convite inicial, senha definida pela pessoa convidada, primeiro login, saída/novo login e sessão após recarregar; formulário de convites acessível.
...
- [x] Validar por API a recuperação de senha hospedada com conta descartável e a rejeição de token inválido pela interface.
- [ ] Validar convite de participantes pela interface, uso único e expiração real do prazo do convite.
